...
/**
 * <p>
 * When a build is performed, LoadRunner Controller and Analysis are being
 * executed on slave machine. When test is finished, averages from Analysis
 * report are reported in jUnit XML report format. Transactions with high
 * response time (above defined limit) are reported as failed. Transactions with
 * high errors rate (above defined limit) are reported as failed. (Not
 * implemented yet)
 *
...
public class LoadRunnerWrapperJenkins extends Builder {
	...
	public LoadRunnerWrapperJenkins(// String name,
			String loadRunnerBin, String loadRunnerScenario, String loadRunnerResultsFolder,
			String loadRunnerAnalysisTemplateName, String loadRunnerAnalysisHTMLReportFolder,
			String loadRunnerResultsSummaryFile, String loadRunnerControllerAdditionalAttributes,
	...
	public ArrayList<LoadRunnerTransactionBoundary> getReportTargetsValuesPerTransaction() {
		return this.reportTargetsValuesPerTransaction;
	}
	...
	public boolean perform(AbstractBuild build, Launcher launcher, BuildListener listener) {
		...
		try {
			...
			remoteLauncher.init(buildNumber, workspacePath, loadRunnerBin, loadRunnerScenario,
					loadRunnerControllerAdditionalAttributes, loadRunnerResultsFolder, loadRunnerAnalysisTemplateName,
					loadRunnerAnalysisHTMLReportFolder, loadRunnerResultsSummaryFile,
					loadRunnerResultsSummaryFileFormat, reportTargetsValuesPerTransaction, acceptedFailurePercentage);
	...
	/**
	 * Descriptor for {@link LoadRunnerWrapperJenkins}. Used as a singleton. The
	 * class is marked as public so that it can be accessed from views.
	 *
	 * <p>
	 * See
	 * <tt>src/main/resources/hudson/plugins/hello_world/LoadRunnerWrapperJenkins/*.jelly</tt>
	...
	public static final class DescriptorImpl extends BuildStepDescriptor<Builder> {
		...
		/**
		 * This method returns accepted failure percentage from the global
		 * configuration
		 */
		...
		/**
		 * This method returns default Transaction Error Value from the global
		 * configuration
		 */
		...
		public FormValidation doCheckLoadRunnerBin(@QueryParameter String value) throws IOException, ServletException {
			if (StringUtils.trimToNull(value) == null || value.length() == 0) {
				return FormValidation.error(
						"LoadRunner bin folder should not be empty. For example: C:\\Program Files (x86)\\HP\\LoadRunner\\bin");
			}
			return FormValidation.ok();
		}

		public FormValidation doCheckLoadRunnerScenario(@QueryParameter String value)
				throws IOException, ServletException {
			if (StringUtils.trimToNull(value) == null || value.length() == 0) {
				return FormValidation.error(
						"LoadRunner scenario path should not be empty. For example: C:\\scenario\\Scenario1.lrs");
			}
		...
		public FormValidation doCheckLoadRunnerControllerAdditionalAttributes(@QueryParameter String value)
				throws IOException, ServletException {
			if (StringUtils.trimToNull(value) != null && value.length() > 0) {
				...
				if (parts.length % 2 != 0) {
					return FormValidation.error(
							"Additional attributes should be supplied in pairs or empty. For example -para1 value1 -param2 value2");
				}
		...
		public FormValidation doCheckLoadRunnerResultsFolder(@QueryParameter String value)
				throws IOException, ServletException {
			if (StringUtils.trimToNull(value) == null || value.length() == 0) {
				return FormValidation
						.error("Results folder should no be empty. For example: %WORKSPACE%\\%BUILD_NUMBER%");
			}
			return FormValidation.ok();
		}

		public FormValidation doCheckLoadRunnerAnalysisTemplateName(@QueryParameter String value)
				throws IOException, ServletException {
			// if (StringUtils.trimToNull(value) == null || value.length() ==
			// 0){
			// return FormValidation.error("");
			// }
			return FormValidation.ok();
		...
		public FormValidation doCheckLoadRunnerAnalysisHTMLReportFolder(@QueryParameter String value)
				throws IOException, ServletException {
			if (StringUtils.trimToNull(value) == null || value.length() == 0) {
				return FormValidation.error(
						"Path to LoadRunner html report should not be empty. Please supply the part as it configured in Load Runner Analysis template");
			}
			return FormValidation.ok();
		}
	}
	...
	public static class LoadRunnerTransactionBoundary extends AbstractDescribableImpl<LoadRunnerTransactionBoundary>
		...
		@DataBoundConstructor
		public LoadRunnerTransactionBoundary(final String transactionName, final float transactionErrorValue,
				final boolean doNotCompare) {
			super();
		...
		public String toString() {
			return " transactionName=" + this.transactionName + " | transactionErrorValue=" + this.transactionErrorValue
					+ " | doNotCompare=" + this.doNotCompare;
		}
